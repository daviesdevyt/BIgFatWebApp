...
@login_required
def fullz(request):
    return render(request, "base/fullz.html", {"fullzz": Fullz.objects.filter(purchased=False).order_by("date_created")})
@login_required
def dumps(request):
...
def logs(request, title):
    ...
    return render(request, "base/logs.html", {"data": classes[title].objects.filter(purchased=False).order_by("date_created"), "name": title})
